...
export const getActivePrompt = async (req, res) => {
  try {
    const { type } = req.params;
    const { model } = req.query; // Accept model as a query param for future use
    ...
    if (!prompt) {
      ...
      switch (type) {
        ...
        case 'conversation':
          ...
          break;
        case 'evaluation':
          // Optionally, you could use the model param here in the future
      ...
      return res.json({ 
        ...
        type,
        model: model || undefined // Echo model if provided
      });
    }
